...
export default function Sidebar({ selectedTopicId, onSelectTopic, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <nav
        id="topic-sidebar"
        aria-label="Topics"
        className={`fixed inset-y-0 left-0 z-40 flex w-[85%] max-w-xs flex-col border-r border-paper-border bg-paper-surface transition-transform duration-200 ease-out dark:border-night-border dark:bg-night-surface md:static md:z-auto md:w-72 md:max-w-none md:translate-x-0 ${
      ...
      >
        ...
        <div className="space-y-3 p-4">
          <SearchBar value={query} onChange={setQuery} resultCount={filteredTopics.length} />
        </div>
                          ...
                          >
                            ...
                            <span
                              className={`block text-xs ${
                                isSelected ? 'text-pine-50' : 'text-ink-muted dark:text-cream-muted'
                              }`}
                            >
                              {topic.summary}
                            </span>
